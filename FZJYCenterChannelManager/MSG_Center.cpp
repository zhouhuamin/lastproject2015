...
{
    CMyLog::m_pLog->_XGSysLog("recv sequence %s,ctrl cmd \n, %s \n", szSequenceNo, szCtrlXML);
...
static ACE_THR_FUNC_RETURN ThreadHandleMySql(void *lParam)
{
	CMSG_Center* pTask = (CMSG_Center*) lParam;
	pTask->ProcessMySql();
	return 0;
...
int CMSG_Center::open()
{
	...
	ACE_Thread_Manager::instance()->spawn(ThreadHandleMySql, this);
...
int CMSG_Center::ConnectToEventServer()
{
    ...
    while (1)
    {
        ...
        {
            if (handler->ConnectToAlarmServer(CSimpleConfig::EVENT_SERVER_PORT, CSimpleConfig::EVENT_SERVER_IP) == -1)
            {
...
int CMSG_Center::ProcessMySql()
{
	//CControlCmdHandler * handler = new CControlCmdHandler;
	while (1)
	{
		structModifyInfo modifyInfo;
		queryModifyRecord(modifyInfo);

		if (modifyInfo.SEQ_NO != "")
		{
			UpdateModifyRecord(modifyInfo);

			if (modifyInfo.CLIENT_FINISH_FLAG == "1" && modifyInfo.MODIFY_FREE_FLAG == "0")
			{

				// send to customs

				UpdateSendFlag(modifyInfo);
			}
			else if (modifyInfo.CLIENT_FINISH_FLAG == "1" && modifyInfo.MODIFY_FREE_FLAG == "1")
			{
				SendPassInfo(modifyInfo);
			}
		}
...
int CMSG_Center::handleUploadCustomsData(NET_PACKET_MSG* pPacket)
{
    ...
    //if (pUploadDataReq->is_data_full != 0)
    //{
    //    return -1;
    //}
...

	{
		...
		m_pPlatformProxy->PackUploadData(pDatabase, "CUSTOMS", pUploadDataReq->szCustomsData, pUploadDataReq->nCustomsDataLen, szPacketData, nPacketLen);
	}
    ...
    if (CSimpleConfig::m_StaPlatformMap.size() > 0)
    {
        ...
        for (iter = CSimpleConfig::m_StaPlatformMap.begin(); iter != CSimpleConfig::m_StaPlatformMap.end(); iter++)
        {
            ...
            if (nRet == nPacketLen)
            {
                CMyLog::m_pLog->_XGSysLog("11111send data to platform %s:%d succ %d...\n", pPlatformInfo->platform_ip, pPlatformInfo->platform_port, nRet);
            }
...
int CMSG_Center::handleChannelPassAck(T_CtrlCmd* pCtrlCmd)
{
    ...
    {
        CMyLog::m_pLog->_XGSysLog("channel connect handle is %d\n", channel_connect_handle);
        return -1;
    }
...
int CMSG_Center::handleContaPicData(NET_PACKET_MSG* pPacket)
{
    ...
    char szCmd[512] = {0};
    FILE* fPath = fopen(szSavePath, "rb");
    if (!fPath)
    {
    ...
    char szHourPath[256] = {0};
    sprintf(szHourPath, "%s/%02d/", szTodayPath, tmNow->tm_hour);

    CMyLog::m_pLog->_XGSysLog("current hour path %s\n", szHourPath);
    ...
    for (int i = 0; i < pPicData->nPicNum; i++)
    {
        char szPicFileName[256] = {0};
        int nPicType=pPicData->picInfo[i].nPicType;
        ...
        FILE* pImageFile = fopen(szPicFileName, "wb");
        if (!pImageFile)
        {
        ...
        RecordContaPicInfo(pPicData->szSequenceNo,nPicType,szPicFileName);
...
int CMSG_Center::handleRegatherData(NET_PACKET_MSG* pMsg)
{
    ...
    if (pReGatherReq->nRegatherType == 1) //手工方式
    {
        ...
        strChannelNo = strChannelInfo.substr(11, 10);
        strIEType = strChannelInfo.substr(21, 1);
        ...
        if (CSimpleConfig::m_StaPlatformMap.size() > 0)
        {
            ...
            for (iter = CSimpleConfig::m_StaPlatformMap.begin(); iter != CSimpleConfig::m_StaPlatformMap.end(); iter++)
            {
                ...
                if (nRet == nPacketLen)
                {
                    CMyLog::m_pLog->_XGSysLog("22222send data to platform %s:%d succ %d...\n", pPlatformInfo->platform_ip, pPlatformInfo->platform_port, nRet);
                }
...
int CMSG_Center::RecordContaPicInfo(char* szSeqNo,int nPicType,char* szFileName)
{
...
int CMSG_Center::queryModifyRecord(structModifyInfo & modifyInfo)
{
	CppMySQL3DB* pDatabase = NULL;
	CMySQLConnectionPool::GetDataAccess(pDatabase);
	...
	char chQuerySQL[1024] = {0};
	sprintf(chQuerySQL, "select AREA_ID,CHANNEL_NO,SEQ_NO,I_E_TYPE,USER_NAME,\
...
int CMSG_Center::UpdateModifyRecord(structModifyInfo & modifyInfo)
{
	CppMySQL3DB* pDatabase = NULL;
	CMySQLConnectionPool::GetDataAccess(pDatabase);
...
int CMSG_Center::UpdateSendFlag(structModifyInfo & modifyInfo)
{
	CppMySQL3DB* pDatabase = NULL;
	CMySQLConnectionPool::GetDataAccess(pDatabase);
	...
	char chQuerySQL[1024] = {0};
	sprintf(chQuerySQL, "UPDATE TS_CHANNEL_GC SET SENDED_CUSTOM_FLAG=1,SENDED_CUSTOM_TIME=now() WHERE SEQ_NO='%s'", modifyInfo.SEQ_NO.c_str());
	pDatabase->execSQL(chQuerySQL);
	return 0;
}

int CMSG_Center::UpdateRecvFlag(structModifyInfo & modifyInfo)
{
	CppMySQL3DB* pDatabase = NULL;
	CMySQLConnectionPool::GetDataAccess(pDatabase);
	...
	char chQuerySQL[1024] = {0};
	sprintf(chQuerySQL, "UPDATE TS_CHANNEL_GC SET RECIEVED_CUSTOM_FLAG=1,RECIEVED_CUSTOM_TIME=now() WHERE SEQ_NO='%s'", modifyInfo.SEQ_NO.c_str());
	pDatabase->execSQL(chQuerySQL);
	return 0;
}

int CMSG_Center::SendPassInfo(structModifyInfo &modifyInfo)
{
	const char packetHead[4] = {(char)0xE2, (char)0x5C, (char)0x4B, (char)0x89};
	const char packetEnd[2]  = {(char)0xFF, (char)0xFF};

	char szRetXmlData[4096]={0};
	std::string	strControlData = "";
	strControlData	= \
		"<CHECK_RESULT>00000000000000000000</CHECK_RESULT>\r\n"
		"<OP_HINT>直接放行</OP_HINT>\r\n"
		"<GPS>\r\n"
		"<VE_NAME></VE_NAME>\r\n"
		"<GPS_ID></GPS_ID>\r\n"
	...
	sprintf(szRetXmlData, "%s", "<?xml version=\"1.0\" encoding=\"GB2312\" ?>\r\n");
	sprintf(szRetXmlData, "%s<COMMAND_INFO AREA_ID=\"%s\" CHNL_NO=\"%s\" I_E_TYPE=\"%s\" SEQ_NO=\"%s\">", szRetXmlData, \
				modifyInfo.AREA_ID.c_str(), modifyInfo.CHNL_NO.c_str(), modifyInfo.IE_FLAG.c_str(), modifyInfo.SEQ_NO.c_str());
	sprintf(szRetXmlData, "%s %s", szRetXmlData, strControlData.c_str());


	int nXmlLen = strlen(szRetXmlData);
	int PackDataLen = 34 + 4 + nXmlLen + 2;

	boost::shared_array<BYTE> spPacketData;
	spPacketData.reset(new BYTE[PackDataLen + 1]);
	memset(spPacketData.get(), 0x00, PackDataLen + 1);
	...
	memcpy(p, modifyInfo.AREA_ID.c_str(), modifyInfo.AREA_ID.size());
	p += 10;

	memcpy(p, modifyInfo.CHNL_NO.c_str(), modifyInfo.CHNL_NO.size());
	p += 10;

	char szIEFlag[2] = {0};
	if (!modifyInfo.IE_FLAG.empty())
		szIEFlag[0] = modifyInfo.IE_FLAG[0];
	...
	//按照 self平台，对数据重新打包
	int send_socket = SysUtil::CreateSocket();
	if (SysUtil::ConnectSocket(send_socket, "127.0.0.1", CSimpleConfig::CUSTOMS_PLATFORM_LISTEN_PORT) != 0)
	{
		SysUtil::CloseSocket(send_socket);
		CMyLog::m_pLog->_XGSysLog("********connect to self platform %s:%d fail...\n", "127.0.0.1", CSimpleConfig::CUSTOMS_PLATFORM_LISTEN_PORT);
		return -1;
	}

	int nRet = SysUtil::SocketWrite(send_socket, (char*)spPacketData.get(), PackDataLen, 1000);

	if (nRet == PackDataLen)
	{
		CMyLog::m_pLog->_XGSysLog("send data to self platform %s:%d succ %d...\n", "127.0.0.1", CSimpleConfig::CUSTOMS_PLATFORM_LISTEN_PORT, nRet);
	}
