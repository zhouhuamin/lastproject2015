...
int CJZPlatformProxy::PackUploadData0X32(CppMySQL3DB* pDatabase, char* szPlatName, char* pData, int nLen, char* szPackedData, int& nPacketDataLen,int nIsRegather) {
	if (strcmp(szPlatName, "CUSTOMS") == 0) {
		m_pBasicDataAccess->RecordPassVehicleInfo(pDatabase, pData,nIsRegather);
		memcpy(szPackedData, pData, nLen);
		nPacketDataLen = nLen;
...
int CJZPlatformProxy::UnpackCtrlData(CppMySQL3DB* pDatabase, char* pData, int nLen) 
{
	...
	case 0x31:	// 卡口后台系统向卡口前端集成系统发出的设备操作控制指令，流程上对应0x32
		ProcessPacket0X31(pDatabase, pData, nLen);
		break;
...
int CJZPlatformProxy::ProcessPacket0X31(CppMySQL3DB* pDatabase, char* pData, int nLen)
{
	char szPlatName[32] = {0};
