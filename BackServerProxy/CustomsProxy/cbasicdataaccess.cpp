...
int CBasicDataAccess::RecordPassVehicleInfo(CppMySQL3DB* pDataabse, char* szXML,int nIsRegather)
{
    TiXmlDocument xml;
    xml.Parse(szXML + 38);
...
int CBasicDataAccess::RecordMsg0X32(CppMySQL3DB* pDataabse, char* szXML,int nIsRegather)
{
	TiXmlDocument xml;
	xml.Parse(szXML + 38);
	...
	if (szXML)
	{
		handleMsg0X32(pAreaID, pChannelNO, pIEType, pSquenceNo, pDataabse, szXML + 38,nIsRegather);
	}
...
int CBasicDataAccess::RecordPassResult0X31(CppMySQL3DB* pDataabse, char* szXML, T_PassResultInfo0X31* pPassResult)
{
	TiXmlDocument xml;
	xml.Parse(szXML + 38);
	...
	if (szXML)
	{
		handleResultAll0X31(pAreaID, pChannelNO, pIEType, pSquenceNo, pDataabse, szXML + 38);
	}


	char EXCUTE_COMMAND[32] = {0};

	TiXmlElement *item_result = root->FirstChildElement("EXCUTE_COMMAND");
	if (item_result)
	{
		char* pText = (char*) item_result->GetText();
		if (pText)
		{
			strcpy(EXCUTE_COMMAND, pText);
		}
		else
		{
			strcpy(EXCUTE_COMMAND, "");
		}
	}

	char OP_TYPE[10] = {0};
	TiXmlElement *item_hint = root->FirstChildElement("OP_TYPE");
	if (item_hint)
	{
		char* pText			= (char*) item_hint->GetText();
		if (pText)
		{
			strcpy(OP_TYPE, pText);
		}
		else
		{
			strcpy(OP_TYPE, "");
	...
	char OP_REASON[1024] = {0};
	TiXmlElement *item_reason = root->FirstChildElement("OP_REASON");
	if (item_reason)
	{
		char* pText = (char*) item_reason->GetText();
		if (pText)
		{
			strcpy(OP_REASON, pText);
		}
		else
		{
			strcpy(OP_REASON, "");
	...
	char OP_ID[64] = {0};
	TiXmlElement *item_formid = root->FirstChildElement("OP_ID");
	if (item_formid)
	{
		char* pText = (char*) item_formid->GetText();
		if (pText)
	...
	strcpy(pPassResult->EXCUTE_COMMAND, EXCUTE_COMMAND);
	strcpy(pPassResult->OP_TYPE,	OP_TYPE);
	strcpy(pPassResult->OP_REASON,	OP_REASON);
	strcpy(pPassResult->OP_ID,		OP_ID);
	strcpy(pPassResult->szSealID,	seal_id);
	strcpy(pPassResult->szSealKey,	seal_key);
	//strcpy(pPassResult->szOpenTimes, open_times);
	...
	return handlePassResult0X31(*pPassResult, pDataabse);

}

int CBasicDataAccess::RecordPassResult0X39(CppMySQL3DB* pDataabse, char* szXML, T_PassResultInfo0X39* pPassResult)
{
	TiXmlDocument xml;
	xml.Parse(szXML + 38);
	...
	if (szXML)
	{
		handleResultAll0X39(pAreaID, pChannelNO, pIEType, pSquenceNo, pDataabse, szXML + 38);
	}
	...
	char EXC_TIME[10] = {0};
	TiXmlElement *item_hint = root->FirstChildElement("EXC_TIME");
	if (item_hint)
	{
		char* pText			= (char*) item_hint->GetText();
		if (pText)
		{
			strcpy(EXC_TIME, pText);
		}
		else
		{
			strcpy(EXC_TIME, "");
	...
	memset(pPassResult, 0, sizeof (T_PassResultInfo0X39));
	...
	strcpy(pPassResult->EXCUTE_COMMAND, EXCUTE_COMMAND);
	strcpy(pPassResult->EXC_TIME,		EXC_TIME);

	return handlePassResult0X39(*pPassResult, pDataabse);
}
...
int CBasicDataAccess::handleAll(char* area_id, char* chnl_no, char* ie_type, char* seq_no, CppMySQL3DB* pDatabase, char* szXML,int nIsRegather)
{
    ...
    {
        char chQuerySQL[1024 * 10] = {0};
        sprintf(chQuerySQL, "UPDATE T_GATHERINFO SET IS_REGATHER =2 , XML_INFO = '%s' WHERE SEQUENCE_NO='%s' "
                , szXML
                , seq_no);
...
int CBasicDataAccess::handleMsg0X32(char* area_id, char* chnl_no, char* ie_type, char* seq_no, CppMySQL3DB* pDatabase, char* szXML,int nIsRegather)
{
	char cpYearMonDay[256] = {0};
	struct tm* tmNow;
	time_t tmTime = time(NULL);
	...
	char chQuerySQL[1024 * 10] = {0};
	sprintf(chQuerySQL, "UPDATE T_GATHERINFO_RESULTXML SET XML_RESULT_INFO = '%s' WHERE SEQUENCE_NO='%s' "
		, szXML
		, seq_no);
...
int CBasicDataAccess::handleResultAll0X31(char* area_id, char* chnl_no, char* ie_type, char* seq_no, CppMySQL3DB* pDatabase, char* szXML)
{
	char cpYearMonDay[256] = {0};
	struct tm* tmNow;
	time_t tmTime = time(NULL);
...
int CBasicDataAccess::handleResultAll0X39(char* area_id, char* chnl_no, char* ie_type, char* seq_no, CppMySQL3DB* pDatabase, char* szXML)
{
	char cpYearMonDay[256] = {0};
	struct tm* tmNow;
	time_t tmTime = time(NULL);
	...
	char chQuerySQL[1024 * 10] = {0};
	sprintf(chQuerySQL, "INSERT INTO T_GATHERINFO_RESULTXML(SEQUENCE_NO, GATHER_AREA, CHANNEL_NO, PASS_TIME, XML_MAN_INFO) VALUES('%s','%s','%s','%s','%s')"
		, seq_no
		, area_id
		, chnl_no
		, cpYearMonDay
	...
	{
		char chQuerySQL[1024 * 10] = {0};
		sprintf(chQuerySQL, "UPDATE T_GATHERINFO_RESULTXML SET XML_MAN_INFO = '%s' WHERE SEQUENCE_NO='%s' "
			, szXML
			, seq_no);
...
int CBasicDataAccess::handlePassResult(T_PassResultInfo& resultInfo, CppMySQL3DB* pDatabase)
{
    ...
    sprintf(chQuerySQL, "INSERT INTO T_GATHERINFO_RESULT(SEQUENCE_NO, GATHER_AREA, CHANNEL_NO, PASS_TIME, CHECK_RESULT, SEAL_ID, SEAL_KEY, OPEN_TIMES, FORM_ID, OP_HINT) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
            , resultInfo.szSeqNo
            , resultInfo.szAeraID
            , resultInfo.szChlNo
            , cpYearMonDay
...
int CBasicDataAccess::handlePassResult0X31(T_PassResultInfo0X31& resultInfo, CppMySQL3DB* pDatabase)
{
	char cpYearMonDay[256] = {0};
	struct tm* tmNow;
	time_t tmTime = time(NULL);
	...
	sprintf(chQuerySQL, "INSERT INTO T_GATHERINFO_RESULT(SEQUENCE_NO, GATHER_AREA, CHANNEL_NO, PASS_TIME, EXCUTE_COMMAND, SEAL_ID, SEAL_KEY, OP_TYPE, OP_REASON, OP_ID) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"
		, resultInfo.szSeqNo
		, resultInfo.szAeraID
		, resultInfo.szChlNo
		, cpYearMonDay
		, resultInfo.EXCUTE_COMMAND
		, resultInfo.szSealID
		, resultInfo.szSealKey
		, resultInfo.OP_TYPE
		, resultInfo.OP_REASON
		, resultInfo.OP_ID);
...
int CBasicDataAccess::handlePassResult0X39(T_PassResultInfo0X39& resultInfo, CppMySQL3DB* pDatabase)
{
	char cpYearMonDay[256] = {0};
	struct tm* tmNow;
	time_t tmTime = time(NULL);
	...
	char chQuerySQL[1024 * 4] = {0};
	sprintf(chQuerySQL, "INSERT INTO T_GATHERINFO_RESULT(SEQUENCE_NO, GATHER_AREA, CHANNEL_NO, PASS_TIME, EXCUTE_COMMAND, EXC_TIME) VALUES('%s','%s','%s','%s','%s','%s')"
		, resultInfo.szSeqNo
		, resultInfo.szAeraID
		, resultInfo.szChlNo
		, cpYearMonDay
	...
	if (nRet != 1)
	{
		...
		sprintf(chQuerySQL, "UPDATE T_GATHERINFO_RESULT SET EXCUTE_COMMAND = '%s', EXC_TIME = '%s', PASS_TIME = '%s' WHERE SEQUENCE_NO='%s' "
			, resultInfo.EXCUTE_COMMAND
			, resultInfo.EXC_TIME
			, cpYearMonDay
			, resultInfo.szSeqNo);
	...
	memset(chQuerySQL, 0x00, sizeof(chQuerySQL));
	sprintf(chQuerySQL, "UPDATE T_GATHERINFO SET PASS_MODE =1 WHERE SEQUENCE_NO='%s' ", resultInfo.szSeqNo);
...
int CBasicDataAccess::RecordPassResult(CppMySQL3DB* pDataabse,char* szSequenceNo,int nResult,char* szHint, char *szEventID, char *szXmlInfo)
{
