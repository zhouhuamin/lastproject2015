...
public:
	...
	bool ReadUploadData(const std::vector<BYTE> &dataVectm, std::pair<BYTE, structUploadData> &pairData);

	uint16 crc16(BYTE *puchMsg, uint16 usDataLen);  

public:
	void ReadInputDiscreteRequest(std::vector<BYTE> &dataVect);
	bool ReadInputDiscreteResponse(const std::vector<BYTE> &dataVect);

	void WriteCoilRequestRising(std::vector<BYTE> &dataVect, BYTE bM, BYTE CH);
	bool WriteCoilResponse(const std::vector<BYTE> &dataVect);
	void WriteCoilRequestOff(std::vector<BYTE> &dataVect, BYTE bM, BYTE CH);
	void WriteRegisterRequest(std::vector<BYTE> &dataVect, BYTE val);
};
