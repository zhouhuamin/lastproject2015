...
static int net_system_CTRL_handle(int server_fd, u8 *buf, int len);
	...
	while (1)
	{
		...
		if (ret > 0) //get data
		{
			...
			case MSG_TYPE_CTRL_EVENT:
				net_system_CTRL_handle(server_fd, net_packet.msg_body,
		...
		{
			net_disconnected = 0;
			server_retry_connect_handle(&server_fd);
