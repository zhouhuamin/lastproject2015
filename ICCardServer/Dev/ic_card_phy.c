		...
		int time_out)
{
	...
	{
		LOG("get the data len error! %d", p_frame->data_len);
		return -1;
	}
	...
	{
		LOG("tail chk timeout!\n");
		return -2;
	}

	if (0 != ic_dev_package_check(p_frame))
	{
		LOG("package chk timeout!\n");
		return -1;
	}
