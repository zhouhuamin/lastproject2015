    ...
    while (1) {
        ...
        if (!CCascadeCmdHandler::m_bConnected) 
		{
            ...
            if (handler->ConnectToCascadeServer(CSimpleConfig::CHANNEL_CONTROLER_PORT, const_cast<char*>(CSimpleConfig::m_strChannelControlIP.c_str())) == -1) 
			{
				...
				//g_ObjectStatus.szObjectStatus			= "011";
			...
			else 
			{
				...
				//g_ObjectStatus.nServiceType				= 0;
				//g_ObjectStatus.szObjectCode				= "DEV_DETECTOR_001";
				//g_ObjectStatus.szObjectStatus			= "010";
				//g_ObjectStatus.szConnectedObjectCode	= "COMM_CONTROL_001";
...
int CMSG_Handle_Center::handleICReaderData(NET_PACKET_MSG *pMsg) {
    ...
    if (m_nCascadeSocket > 0) {
        ...
        if (nRet == sendLen) 
		{
            //CMyLog::m_pLog->_XGSysLog("send conta reader data to device server succ...\n");
			...
			g_ObjectStatus.nServiceType				= 0;
			g_ObjectStatus.szObjectCode				= "DEV_DETECTOR_001";
			g_ObjectStatus.szObjectStatus			= "000";
			g_ObjectStatus.szConnectedObjectCode	= "COMM_CONTROL_001";
...
int CMSG_Handle_Center::RegisterToDeviceServer() {
    ...
    strcpy(pRegisterReq->device_tag, "DETECTOR");
    ...
    if (nRet == sendLen) 
	{
        //CMyLog::m_pLog->_XGSysLog("register to device server succ...\n");
		syslog(LOG_DEBUG, "register to device server succ...\n");

    } 
	else 
	{
        //CMyLog::m_pLog->_XGSysLog("********egister to device server fail,to send is %d ,send is %d...\n", sendLen, nRet);
		syslog(LOG_DEBUG, "********egister to device server fail,to send is %d ,send is %d...\n", sendLen, nRet);
    }

}

std::string CMSG_Handle_Center::GetCurTime(void)
{
	time_t t		= time(0);
	tm *ld			= NULL;
	char tmp[64]	= "";
