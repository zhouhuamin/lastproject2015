...
int CMSG_Center::HandleNetMessage(NET_PACKET_MSG *pPacket)
{
    ...
    switch (pPacket->msg_head.msg_type)
    {
		...
		case SYS_MSG_MAN_FANGXI:
			handleManFangXi(pPacket);
			break;
		default:
			break;
    }
...
int CMSG_Center::handleUploadCustomsData(NET_PACKET_MSG* pPacket)
{
    ...
    //if (CSimpleConfig::m_StaPlatformMap.size() > 0)
    {
        ...
        //for (iter = CSimpleConfig::m_StaPlatformMap.begin(); iter != CSimpleConfig::m_StaPlatformMap.end(); iter++)
        {
			...
			int send_socket = SysUtil::CreateSocket();
			if (SysUtil::ConnectSocket(send_socket, CSimpleConfig::BACK_SERVER_IP, CSimpleConfig::BACK_SERVER_PORT) != 0)
			{
			    SysUtil::CloseSocket(send_socket);
			    CMyLog::m_pLog->_XGSysLog("********connect to backer server %s:%d fail...\n", CSimpleConfig::BACK_SERVER_IP, CSimpleConfig::BACK_SERVER_PORT);
			    return -1;
			}
			char chBuf[32] = {0};
			...
			nRet += SysUtil::SocketWrite(send_socket, (char*) pPacket, sizeof (NET_PACKET_HEAD) + pPacket->msg_head.packet_len, 50);

			memcpy(chBuf, SYS_NET_MSGTAIL, 8);
			nRet += SysUtil::SocketWrite(send_socket, chBuf, 8, 50);

			if (nRet == 8 + 8 + sizeof (NET_PACKET_HEAD) + pPacket->msg_head.packet_len)
			{
				CMyLog::m_pLog->_XGSysLog("send to backer server succ...\n");
			} 
			else //数据发送不成功
			{
				CMyLog::m_pLog->_XGSysLog("send to backer server fail...\n");
			}
			SysUtil::CloseSocket(send_socket);
        }
    }
...
int CMSG_Center::handleCustomsDataAck(NET_PACKET_MSG* pPacket)
{
	...
	char szAreaNo[20]	 = {0};
	char szChannelNo[20] = {0};
	char szIEFlag[10]	 = {0};
	char szSeqNo[30]	 = {0};

    m_pPlatformProxy->UnpackCtrlData(pDatabase, pCustomsDataAck->szCustomsData, pCustomsDataAck->nCustomsDataLen, szAreaNo, szChannelNo, szIEFlag, szSeqNo);

	// 	// 0x32.
	send_packet_0x32(pCustomsDataAck, szAreaNo, szChannelNo, szIEFlag, szSeqNo);

    return 0;
}

int CMSG_Center::handleManFangXi(NET_PACKET_MSG* pPacket)
{
	T_Upload_Customs_Data* pCustomsDataAck = (T_Upload_Customs_Data*) pPacket->msg_body;
...
int CMSG_Center::send_packet_0x32(T_Upload_Customs_Data* pCustomsDataAck, char *pAreaNo, char *pChannelNo, char *pIE, char *pSeqNo)
{
	if (pCustomsDataAck == NULL)
		return -1;

	char szXMLCommandInfo[1024] = {0};
	sprintf(szXMLCommandInfo, "<COMMAND_INFO AREA_ID=\"%s\" CHNL_NO=\"%s\" I_E_TYPE=\"%s\" SEQ_NO=\"%s\">",
		pAreaNo, pChannelNo, pIE, pSeqNo);
	sprintf(szXMLCommandInfo, "%s\n<EXECUTE_RESULT></EXECUTE_RESULT>\n",szXMLCommandInfo);
	sprintf(szXMLCommandInfo, "%s\n<ESEAL_USE_COUNT></ESEAL_USE_COUNT>\n",szXMLCommandInfo);
	sprintf(szXMLCommandInfo, "%s\n</COMMAND_INFO>\n",szXMLCommandInfo);


	//CMyLog::m_pLog->_XGSysLog("send 0x32 data to backer server %s...\n", pCustomsDataAck->szSequenceNo);
	...
	szSendPacket[nLen] = 0X32;
	nLen += sizeof (char);

	memcpy(szSendPacket + nLen, pAreaNo, 10);
	nLen += 10;

	memcpy(szSendPacket + nLen, pChannelNo, 10);
	nLen += 10;
	...
	int nXMLLen = strlen(szXMLCommandInfo) + 1;
	...
	strcpy(szSendPacket + nLen, szXMLCommandInfo);
	nLen += nXMLLen;
	...
	pPacket_->msg_head.msg_type = SYS_MSG_CONTROL_RESULT_ACK;
	...
	sprintf(pCustomsData->szChannelNo, "%s%s%s", pAreaNo, pChannelNo, pIE);
	strcpy(pCustomsData->szSequenceNo, pCustomsDataAck->szSequenceNo);
	...
	{
		CMyLog::m_pLog->_XGSysLog("send 0x32 to backer server succ...\n");
	} 
	else //数据发送不成功
	{
		CMyLog::m_pLog->_XGSysLog("send 0x32 to backer server fail...\n");
	}
	SysUtil::CloseSocket(send_socket);
