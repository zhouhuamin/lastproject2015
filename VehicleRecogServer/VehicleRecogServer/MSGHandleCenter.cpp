...
int CMSG_Handle_Center::handleICReaderData(NET_PACKET_MSG *pMsg) {
    ...
    if (m_nCascadeSocket > 0) {
        ...
        if (nRet == sendLen) {
            ...
            syslog(LOG_DEBUG, "send opt_car data to device server succ...\n");

        } else {

            syslog(LOG_DEBUG, "********send opt_car data to device server fail,to send is %d ,send is %d...\n", sendLen, nRet);
	...
	for (int i = 0; i < CSimpleConfig::m_eventVect.size(); ++i)
	{
		if (strcmp(CSimpleConfig::m_eventVect[i].event_id, pMsg->msg_body) == 0)
		{
			...
			if (strcmp(CSimpleConfig::m_eventVect[i].sequence_no, "1") == 0)
			{
				...
				// EC_START_OPTCAR
				//sleep(3);
				g_VehicleSemStart.post();
				break;
			}
			else if (strcmp(CSimpleConfig::m_eventVect[i].sequence_no, "2") == 0)
			{
				// EC_STOP_OPTCAR
				pthread_mutex_lock (&g_VehicleDataStackMutex);
				g_nShield	= 0;
				pthread_mutex_unlock (&g_VehicleDataStackMutex);
				g_VehicleSemStop.post();
				break;
			}
			else if (strcmp(CSimpleConfig::m_eventVect[i].sequence_no, "3") == 0)
			{
				// EG_FINISHED_OPTCAR	
				pthread_mutex_lock (&g_VehicleDataStackMutex);
				g_nShield	= 0;
			...
			else if (strcmp(CSimpleConfig::m_eventVect[i].sequence_no, "4") == 0)
			{
				// EG_ARRIVED_OPTCAR
				break;
			}
