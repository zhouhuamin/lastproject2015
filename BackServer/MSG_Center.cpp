...
const char* DATABASENAME = "GatherGateSys_2";
...
int CMSG_Center::HandleNetMessage(NET_PACKET_MSG *pPacket)
{
    ...
    switch (pPacket->msg_head.msg_type)
    {
		...
		case SYS_MSG_CONTROL_RESULT_ACK:
			handleControlResultAck(pPacket);
			break;
...
int CMSG_Center::handleUploadCustomsData(NET_PACKET_MSG* pPacket)
{
    ...
    CMyLog::m_pLog->_XGSysLog("recv fronter server upload customs data %s seq %s to upload ...\n", pUploadDataReq->szChannelNo, pUploadDataReq->szSequenceNo);
...
int CMSG_Center::handleControlResultAck(NET_PACKET_MSG* pPacket)
{
	if (!pPacket)
	{
		return -1;
	...
	CMyLog::m_pLog->_XGSysLog("recv fronter server upload 0x32 data %s seq %s to upload ...\n", pUploadDataReq->szChannelNo, pUploadDataReq->szSequenceNo);
	...
	if (CSimpleConfig::m_StaPlatformMap.size() > 0)
	{
		...
		for (iter = CSimpleConfig::m_StaPlatformMap.begin(); iter != CSimpleConfig::m_StaPlatformMap.end(); iter++)
		{
...

			{
				...
				if (pUploadDataReq->is_data_full == 1)
				{
					m_pPlatformProxy->PackUploadData0X32(pDatabase, szPlatformName, pUploadDataReq->szCustomsData, pUploadDataReq->nCustomsDataLen, szPacketData, nPacketLen, 1);
				} else
				{
					m_pPlatformProxy->PackUploadData0X32(pDatabase, szPlatformName, pUploadDataReq->szCustomsData, pUploadDataReq->nCustomsDataLen, szPacketData, nPacketLen, 0);
				}
			}
...
int CMSG_Center::handleCustomsDataAck(NET_PACKET_MSG* pPacket)
{
	...
	if (pCustomsDataAck->nCustomsDataLen > 30)
		memcpy(szStationNo, pCustomsDataAck->szCustomsData + 9, 10);

	T_MonitorStationRel monitorInfo;
	monitorInfo.AREA_ID = szStationNo;
	QueryMonitorRecord(monitorInfo);

	int nMonitorPort = atoi(monitorInfo.MONITOR_PORT.c_str());
	int send_socket = SysUtil::CreateSocket();
	if (SysUtil::ConnectSocket(send_socket, monitorInfo.MONITOR_IP.c_str(), nMonitorPort) != 0)
	{
		SysUtil::CloseSocket(send_socket);
		//CMyLog::m_pLog->_XGSysLog("********connect to fronter server %s:%d fail...\n", pPlatformInfo->platform_ip, pPlatformInfo->platform_port);
		return -1;
	}
	char chBuf[32] = {0};
	...
	{
		CMyLog::m_pLog->_XGSysLog("send to fronter server succ...\n");
	} 
	else //数据发送不成功
	{
		CMyLog::m_pLog->_XGSysLog("send to fronter server fail...\n");
	}
	SysUtil::CloseSocket(send_socket);
...
int CMSG_Center::handleManFangXi(NET_PACKET_MSG* pPacket)
{
	...
	if (nRet == 8 + 8 + sizeof (NET_PACKET_HEAD) + pPacket->msg_head.packet_len)
	{
		CMyLog::m_pLog->_XGSysLog("send 0x39 to fronter server succ...\n");
	} 
	else //数据发送不成功
	{
		CMyLog::m_pLog->_XGSysLog("send 0x39 to fronter server fail...\n");
	}
	SysUtil::CloseSocket(send_socket);
...
int CMSG_Center::QueryMonitorRecord(T_MonitorStationRel &monitorInfo)
{
	CppMySQL3DB* pDatabase = NULL;
	CMySQLConnectionPool::GetDataAccess(pDatabase);
	...
	sprintf(chQuerySQL, "select MONITOR_IP,MONITOR_PORT from T_MONITOR_STATION_REL where AREA_ID='%s' limit 1", monitorInfo.AREA_ID.c_str());
	...
	std::string strFieldName = "MONITOR_IP";
	const char* content = query1.getStringField(strFieldName.c_str());
	if (content != NULL)
		monitorInfo.MONITOR_IP	= content;

	strFieldName = "MONITOR_PORT";
	content = query1.getStringField(strFieldName.c_str());
	if (content != NULL)
		monitorInfo.MONITOR_PORT	= content;
